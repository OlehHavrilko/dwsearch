#!/usr/bin/env bash
# install.sh — Dwsearch installer for Linux and macOS
# Creates `dwsearch` (web) and `dwsearch-cli` launcher commands.
...
info "Installing dwsearch dependencies..."
$PIP install --quiet -r "$SCRIPT_DIR/requirements.txt"

if [ -f "$SCRIPT_DIR/dwsearch-web/requirements.txt" ]; then
    info "Installing dwsearch-web dependencies..."
    $PIP install --quiet -r "$SCRIPT_DIR/dwsearch-web/requirements.txt"
fi
...
DARKDUMP_LAUNCHER="$INSTALL_DIR/dwsearch"

DARKDUMP_SCRIPT="#!/usr/bin/env bash
# Dwsearch web launcher — generated by install.sh
exec \"$PYTHON\" \"$SCRIPT_DIR/dwsearch-web/app.py\" \"\$@\"
"
...
DARKDUMP_CLI_LAUNCHER="$INSTALL_DIR/dwsearch-cli"

DARKDUMP_CLI_SCRIPT="#!/usr/bin/env bash
# Dwsearch CLI launcher — generated by install.sh
exec \"$PYTHON\" \"$SCRIPT_DIR/dwsearch.py\" \"\$@\"
"
...
echo -e "  ${BOLD}dwsearch${RESET}      — start the web interface (then open http://127.0.0.1:5000)"
echo -e "  ${BOLD}dwsearch-cli${RESET}  — run the CLI tool"
echo ""
